...
import { getProfitReport, getStockValuation, getSalesSummary, getTopProducts, getAuditLogs } from "../api/api";
...
export default function ReportsPage() {
  ...
  const fetchTab = async (tab) => {
    ...
    try {
      ...
      if (tab === "Stock valuation")  result = await getStockValuation();
      if (tab === "Sales summary")    result = await getSalesSummary();
      if (tab === "Audit log")        result = await getAuditLogs();
      setData(result);
    } catch {
    ...
    <div style={{ padding: "16px 24px", overflowY: "auto", height: "100%", boxSizing: "border-box" }}>
        ...
        <>
          <div style={{ ...kpiCard, marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 22, fontWeight: 500, color: "#185FA5" }}>
              ₦{parseFloat(data.total_inventory_value || 0).toLocaleString("en-NG", { minimumFractionDigits: 2 })}
            </span>
          </div>
          <div style={tableWrap}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                {(data.products || []).map((p, i) => (
                  <tr key={i} style={{ borderBottom: "1px solid var(--color-border-tertiary)" }}>
                    ...
                    <td style={{ ...td, textAlign: "right", color: "var(--color-text-secondary)" }}>
                      ₦{parseFloat(p.cost_price || 0).toLocaleString("en-NG", { minimumFractionDigits: 2 })}
                    </td>
      ...
      {!loading && activeTab === "Sales summary" && data && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, maxWidth: 500 }}>
          <StatCard label="Total revenue" value={`₦${parseFloat(data.total_sales || 0).toLocaleString("en-NG", { minimumFractionDigits: 2 })}`} color="#185FA5" />
          <StatCard label="Total transactions" value={data.transactions || 0} color="#0F6E56" />
        </div>
      )}
