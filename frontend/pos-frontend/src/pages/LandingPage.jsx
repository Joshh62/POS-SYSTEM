  ...
  if (signupResult) {
    return (
      <div style={{ minHeight: "100vh", background: "#F8FAFC", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
        <div style={{ background: "#fff", borderRadius: 16, padding: "48px 40px", maxWidth: 480, width: "100%", textAlign: "center", boxShadow: "0 4px 24px rgba(0,0,0,0.08)", border: "1px solid #e5e7eb" }}>
          <div style={{ fontSize: 48, marginBottom: 16 }}>🎉</div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#1a1a1a", marginBottom: 8 }}>You're all set!</h1>
          <p style={{ fontSize: 14, color: "#555", marginBottom: 24, lineHeight: 1.7 }}>
            <strong>{signupResult.business_name}</strong> has been created on the <strong>{signupResult.plan?.charAt(0).toUpperCase() + signupResult.plan?.slice(1)}</strong> plan.<br />
            Your <strong>14-day free trial</strong> has started.
          </p>

          <div style={{ background: "#EAF3DE", border: "1px solid rgba(59,109,17,0.3)", borderRadius: 10, padding: "14px 18px", marginBottom: 24, textAlign: "left" }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: "#3B6D11", marginBottom: 8 }}>Your login details</div>
            <div style={{ fontSize: 13, color: "#1a1a1a" }}>
              <div style={{ marginBottom: 4 }}>
                <span style={{ color: "#555" }}>Username: </span>
                <strong style={{ fontFamily: "monospace" }}>{signupResult.username}</strong>
              </div>
              <div style={{ fontSize: 11, color: "#666", marginTop: 6 }}>
                Save these details. Use them to log in at profittrack.ng
              </div>
            </div>
          </div>
          ...
          <button
            onClick={onStart}
            style={{ width: "100%", padding: "13px 0", borderRadius: 10, border: "none", background: "#185FA5", color: "#fff", fontSize: 15, fontWeight: 600, cursor: "pointer", marginBottom: 12 }}
          >
            Go to my dashboard →
          </button>
    ...
    <div style={page}>
      ...
      <nav style={nav}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <button style={navBtn} onClick={() => setShowSignup(true)}>Start free trial</button>
      ...
      <section style={hero}>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
          <button style={btnPrimary} onClick={() => setShowSignup(true)}>Start 14-day free trial →</button>
          <a href={WHATSAPP_URL} target="_blank" rel="noreferrer" style={btnWhatsapp}>
            📱 Request a free demo
          </a>
        </div>
        <p style={heroNote}>No credit card required. Setup takes less than 5 minutes.</p>
      </section>
      ...
      <section style={{ ...section, background: "#F0F7FF", borderRadius: 0, maxWidth: "100%" }}>
      ...
      <section style={section}>
        ...
        <div style={pricingGrid}>
          {[
            { name: "Solo",       price: "₦5,000",  period: "/month", annual: "₦50,000/year",             desc: "1 owner login. Full POS, inventory, reports. Perfect for solo shop owners.", hot: false },
            { name: "Starter",    price: "₦12,000", period: "/month", annual: "₦120,000/year · save ₦24,000", desc: "1 branch, up to 3 staff. POS, inventory, reports, audit log, loyalty.", hot: false },
            { name: "Business",   price: "₦25,000", period: "/month", annual: "₦250,000/year · save ₦50,000", desc: "Up to 3 branches, 10 users. Full analytics, WhatsApp reports.", hot: true  },
            { name: "Enterprise", price: "₦50,000", period: "/month", annual: "₦500,000/year · save ₦100,000", desc: "Unlimited branches and staff. White-label branding, priority support.", hot: false },
          ].map((p, i) => (
            <div key={i} style={{ ...planCard2, border: p.hot ? "2px solid #185FA5" : "1px solid #e5e7eb", position: "relative" }}>
              ...
              <button
                ...
                style={{ width: "100%", padding: "9px 0", borderRadius: 8, border: p.hot ? "none" : "1px solid #185FA5", background: p.hot ? "#185FA5" : "#fff", color: p.hot ? "#fff" : "#185FA5", fontSize: 13, fontWeight: 600, cursor: "pointer", marginTop: 12 }}
              >
                Start free trial
              </button>
            </div>
      ...
      <section style={ctaSection}>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
          <button style={btnPrimaryWhite} onClick={() => setShowSignup(true)}>Start free trial today</button>
...
const navBtn       = { padding: "7px 16px", borderRadius: 8, border: "1px solid #185FA5", background: "#fff", color: "#185FA5", fontSize: 13, cursor: "pointer", fontWeight: 500 };
...
const planCard2    = { background: "#fff", borderRadius: 12, padding: "20px 18px" };
...
const planDesc     = { fontSize: 12, color: "#555", lineHeight: 1.6, marginTop: 8 };
