...
export default function UsersPage() {
  ...
  // Use active branch if available, else fall back to user's own branch
  const effectiveBranchId = activeBranchId || currentUser.branch_id;
  ...
  const handleCreate = async () => {
    ...
    try {
      await api.post("/auth/register", {
        ...form,
        branch_id:   effectiveBranchId      ?? null,  // ✅ use active branch
  ...
  return (
    <div style={{ padding: "16px 24px", overflowY: "auto", height: "100%", boxSizing: "border-box" }}>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", alignItems: "center", gap: 12, marginBottom: 16 }}>
        ...
        <button onClick={openChangePwd} style={outlineBtn}>
          🔒 Change password
        </button>
        ...
        <div style={overlayStyle}>
          <div style={modalStyle}>
            ...
            <div style={{ background: "#1e2535", border: "1px solid #2a3247", borderRadius: 8, padding: "9px 12px", fontSize: 12, color: "#8a93a6", marginBottom: 18 }}>
              Changing password for <strong style={{ color: "#e8ecf2" }}>@{currentUser.username}</strong>
            </div>
            {pwdSuccess ? (
              <>
              ...
              <>
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  <Field label="Current password">
                    <input type="password" style={inputStyle} value={pwdForm.current} onChange={e => setPwdForm({ ...pwdForm, current: e.target.value })} placeholder="Enter current password" />
                  </Field>
                  <Field label="New password">
                    <input type="password" style={inputStyle} value={pwdForm.newPwd} onChange={e => setPwdForm({ ...pwdForm, newPwd: e.target.value })} placeholder="Min. 6 characters" />
                  </Field>
                  <Field label="Confirm new password">
                    <input type="password" style={inputStyle} value={pwdForm.confirm} onChange={e => setPwdForm({ ...pwdForm, confirm: e.target.value })} placeholder="Repeat new password" />
                  </Field>
                </div>
                {pwdError && <div style={{ ...errorBox, marginTop: 12 }}>{pwdError}</div>}
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 18 }}>
                  <button onClick={handleChangePwd} disabled={pwdLoading} style={{ ...actionBtn, background: pwdLoading ? "#2a3247" : "#185FA5", color: pwdLoading ? "#5a6475" : "#fff", cursor: pwdLoading ? "not-allowed" : "pointer" }}>
          ...
          <div style={upgradeModal}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
              <div>
                ...
                <h2 style={{ fontSize: 18, fontWeight: 700, margin: 0, color: "#e8ecf2" }}>Upgrade to {upgradeInfo.next.label}</h2>
                <p style={{ fontSize: 13, color: "#8a93a6", margin: "6px 0 0" }}>You've used all {maxUsers} staff slot{maxUsers !== 1 ? "s" : ""} on your {upgradeInfo.current.label} plan.</p>
              </div>
              <button onClick={() => setShowUpgrade(false)} style={closeBtn}>×</button>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
              <div style={{ background: "#1a2133", border: "1px solid #2a3247", borderRadius: 10, padding: "12px 14px" }}>
              ...
              <div style={{ background: "linear-gradient(135deg, #0f2744, #1a3a5c)", border: "1px solid #185FA5", borderRadius: 10, padding: "12px 14px" }}>
                ...
                <div style={{ fontSize: 10, color: "#5BA8F5", marginTop: 6, fontWeight: 500 }}>Annual plan saves 2 months</div>
              </div>
            </div>
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: "#c0c7d4", marginBottom: 10 }}>What you get with {upgradeInfo.next.label}:</div>
            ...
            <div style={{ background: "#1a2133", border: "1px solid #2a3247", borderRadius: 8, padding: "10px 14px", marginBottom: 20, fontSize: 12, color: "#8a93a6", lineHeight: 1.6 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <a href={buildWhatsAppLink(planInfo?.plan, businessName)} target="_blank" rel="noreferrer" style={{ display: "block", width: "100%", padding: "12px 0", borderRadius: 10, border: "none", background: "#25D366", color: "#fff", fontSize: 14, fontWeight: 600, cursor: "pointer", textDecoration: "none", textAlign: "center", boxSizing: "border-box" }}>
                💬 Contact us on WhatsApp to upgrade
              </a>
              <a href="tel:08154586355" style={{ display: "block", width: "100%", padding: "10px 0", borderRadius: 10, border: "1px solid #3a4255", background: "none", color: "#c0c7d4", fontSize: 13, fontWeight: 500, cursor: "pointer", textDecoration: "none", textAlign: "center", boxSizing: "border-box" }}>
                📞 Call us: 08154586355
              </a>
              <button onClick={() => setShowUpgrade(false)} style={{ width: "100%", padding: "9px 0", borderRadius: 10, border: "none", background: "none", color: "#5a6475", fontSize: 12, cursor: "pointer" }}>Maybe later</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={overlayStyle}>
          <div style={modalStyle}>
            ...
            <div style={{ background: "#1e2535", border: "1px solid #2a3247", borderRadius: 8, padding: "9px 12px", fontSize: 12, color: "#8a93a6", marginBottom: 18 }}>
              ...
              {/* ✅ Show active branch name, not hardcoded Branch 1 */}
              <strong style={{ color: "#e8ecf2" }}>Branch {effectiveBranchId}</strong>{" "}
              automatically
            </div>
            {formSuccess ? (
              <>
                <div style={successBox}>{formSuccess}</div>
                <button onClick={closeForm} style={{ ...actionBtn, background: "#185FA5", color: "#fff", cursor: "pointer", marginTop: 14 }}>Done</button>
              </>
            ) : (
              <>
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 18 }}>
                  <button onClick={handleCreate} disabled={formLoading} style={{ ...actionBtn, background: formLoading ? "#2a3247" : "#185FA5", color: formLoading ? "#5a6475" : "#fff", cursor: formLoading ? "not-allowed" : "pointer" }}>
                    {formLoading ? "Creating..." : "Create user"}
                  </button>
                  <button onClick={closeForm} style={{ ...actionBtn, background: "none", border: "1px solid #3a4255", color: "#c0c7d4", cursor: "pointer" }}>Cancel</button>
                </div>
              </>
            )}
          </div>
...
const successBox   = { background: "#EAF3DE", color: "#3B6D11", borderRadius: 8, padding: "10px 14px", fontSize: 13, marginBottom: 14 };
...
const upgradeModal = { background: "#151b28", borderRadius: 16, padding: 28, width: "100%", maxWidth: 440, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 24px 64px rgba(0,0,0,0.7)", border: "1px solid #2a3247" };
