...
export default function UsersPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      // ✅ Auto-assign branch_id and business_id from logged-in admin
  ...
  const handleDeactivate = async (userId, username) => {
    ...
    try { await api.patch(`/auth/users/${userId}/deactivate`); fetchUsers(); }
  ...
  const handleActivate = async (userId, username) => {
    ...
    try { await api.patch(`/auth/users/${userId}/activate`); fetchUsers(); }
      ...
      {showForm && (
        <div style={overlayStyle}>
          <div style={modalStyle}>
            ...
            {/* Branch info banner */}
            <div style={{ background: "var(--color-background-secondary)", borderRadius: 8, padding: "8px 12px", fontSize: 11, color: "var(--color-text-secondary)", marginBottom: 14 }}>
              User will be assigned to <strong style={{ color: "var(--color-text-primary)" }}>Branch {currentUser.branch_id}</strong> automatically
            </div>
            ...
            {formSuccess ? (
              <>
                ...
                <button onClick={closeForm} style={{ ...actionBtn, background: "#185FA5", color: "#fff", cursor: "pointer" }}>Done</button>
              </>
            ) : (
              <>
                ...
                <button onClick={handleCreate} disabled={formLoading}
                  style={{ ...actionBtn, background: formLoading ? "var(--color-background-secondary)" : "#185FA5", color: formLoading ? "var(--color-text-tertiary)" : "#fff", cursor: formLoading ? "not-allowed" : "pointer", marginTop: 16 }}>
                  {formLoading ? "Creating..." : "Create user"}
                </button>
                <button onClick={closeForm}
                  style={{ ...actionBtn, background: "none", border: "1px solid var(--color-border-secondary)", color: "var(--color-text-secondary)", marginTop: 8, cursor: "pointer" }}>
                  Cancel
                </button>
              </>
            )}
