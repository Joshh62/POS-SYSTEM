...
const PAGE_TITLES = {
  pos:        "Point of sale",     dashboard:  "Dashboard",
  sales:      "Sales history",     products:   "Products",
  inventory:  "Inventory",         reports:    "Reports",
  analytics:  "Analytics",         users:      "Users",
  businesses: "Businesses",        expenses:   "Expenses",
  customers:  "Customers",         suppliers:  "Suppliers",
  settings:   "Branding & Settings", billing:  "Plan & Billing",
};
...
export default function POSLayout({ children, activePage, onNavigate, onLogout, lastScan }) {
  ...
  // Only show branch switcher on pages where it's relevant
  const canSwitchBranch = ["admin", "superadmin"].includes(role)
    && branches.length > 1
    && isEnabled("multi_branch")
    && !BRANCH_SWITCHER_HIDDEN_PAGES.has(activePage);
  ...
  const handleNavigate = (key) => { onNavigate(key); setDrawerOpen(false); };
      ...
      <div style={{ position: "fixed", inset: 0, display: "flex", flexDirection: "column", overflow: "hidden", background: "var(--color-background-tertiary)" }}>
        ...
        <div style={{ height: 52, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 14px", background: "var(--color-background-primary)", borderBottom: "1px solid var(--color-border-tertiary)", flexShrink: 0, zIndex: 100 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            {canSwitchBranch && (
              <select value={activeBranchId ?? ""} onChange={e => setActiveBranchId(e.target.value ? parseInt(e.target.value) : null)}
            ...
            <button onClick={() => setDrawerOpen(true)} style={{ background: "none", border: "none", cursor: "pointer", padding: "6px", color: "var(--color-text-primary)", fontSize: 18, lineHeight: 1 }}>☰</button>
          </div>
        </div>
        ...
        <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, height: 60, background: "var(--color-background-primary)", borderTop: "1px solid var(--color-border-tertiary)", display: "flex", alignItems: "center", zIndex: 100 }}>
          {bottomNavItems.map(item => {
            ...
            return (
              <button key={item.key} onClick={() => handleNavigate(item.key)} style={{
                flex: 1, height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3,
          ...
          <button onClick={() => setDrawerOpen(true)} style={{
            flex: 1, height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3,
            border: "none", background: "none", cursor: "pointer",
            color: !BOTTOM_NAV_KEYS.includes(activePage) ? "var(--color-primary)" : "var(--color-text-tertiary)",
            borderTop: !BOTTOM_NAV_KEYS.includes(activePage) ? "2px solid var(--color-primary)" : "2px solid transparent",
          }}>
        ...
        {drawerOpen && (
          <>
            ...
            <div style={{ position: "fixed", top: 0, left: 0, bottom: 0, width: 280, background: "var(--color-background-primary)", zIndex: 201, display: "flex", flexDirection: "column", boxShadow: "4px 0 24px rgba(0,0,0,0.2)" }}>
                ...
                {bottomNavItems.map(item => {
                  ...
                  return (
                    <button key={item.key} onClick={() => handleNavigate(item.key)} style={{
                      ...
                      fontWeight: active ? 500 : 400, fontSize: 14, cursor: "pointer", marginBottom: 2,
                    }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    {Object.entries(THEME_DOTS).map(([t, colors]) => (
                      <button key={t} title={colors.label} onClick={() => setTheme(t)} style={{
                        width: 16, height: 16, borderRadius: "50%", border: "none", background: colors[mode], cursor: "pointer", padding: 0,
                        outline: theme === t ? `2px solid ${colors[mode]}` : "none", outlineOffset: 2, transform: theme === t ? "scale(1.2)" : "scale(1)",
                      }} />
                    ))}
      ...
      <div style={{ width: sideW, minWidth: sideW, maxWidth: sideW, height: "100%", background: "var(--color-background-primary)", borderRight: "1px solid var(--color-border-tertiary)", display: "flex", flexDirection: "column", transition: "width 0.18s ease", overflow: "hidden", flexShrink: 0 }}>
          ...
          {visible.map(item => {
            ...
            return (
              <button key={item.key} onClick={() => onNavigate(item.key)} style={{
                ...
                background: active ? "var(--color-primary-light)" : "transparent",
                color: active ? "var(--color-primary)" : "var(--color-text-secondary)",
                fontWeight: active ? 500 : 400, fontSize: 13, cursor: "pointer", width: "100%", transition: "all 0.15s",
              }}
        ...
        <div style={{ borderTop: "1px solid var(--color-border-tertiary)", padding: collapsed ? "10px 0" : "10px 12px" }}>
          ...
          <button onClick={openChangePwd} style={{ display: "flex", alignItems: "center", justifyContent: collapsed ? "center" : "flex-start", gap: 6, background: "none", border: "none", color: "var(--color-text-secondary)", fontSize: 12, cursor: "pointer", padding: "4px 0", width: "100%", marginBottom: 2 }}>
            <span style={{ fontSize: 14 }}>🔒</span>{!collapsed && <span>Change password</span>}
          </button>
          <button onClick={() => setShowSignOut(true)} style={{ display: "flex", alignItems: "center", justifyContent: collapsed ? "center" : "flex-start", gap: 6, background: "none", border: "none", color: "var(--color-danger)", fontSize: 12, cursor: "pointer", padding: "4px 0", width: "100%" }}>
            <span style={{ fontSize: 14 }}>🚪</span>{!collapsed && <span>Sign out</span>}
          </button>
        </div>
      </div>
        ...
        <div style={{ height: 52, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", background: "var(--color-background-primary)", borderBottom: "1px solid var(--color-border-tertiary)", gap: 12 }}>
          ...
          {canSwitchBranch && (
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <select value={activeBranchId ?? ""} onChange={e => setActiveBranchId(e.target.value ? parseInt(e.target.value) : null)}
                style={{ padding: "4px 8px", borderRadius: 6, fontSize: 12, border: "1px solid var(--color-border-tertiary)", background: "var(--color-background-secondary)", color: "var(--color-text-primary)", cursor: "pointer", maxWidth: 200 }}>
