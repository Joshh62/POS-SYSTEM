      ...
      <div style={overlayStyle}>
        <div style={panelStyle}>
          ...
          <div style={summaryBox}>
            ...
            <div style={summarySub}>via {receipt.payment_method}</div>
          </div>
        ...
        <div style={orderBox}>
          {cartItems.map((item) => (
            <div key={item.product_id} style={orderRow}>
              <span style={{ color: "var(--text-h)" }}>{item.product_name} × {item.quantity}</span>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {PAYMENT_METHODS.map((method) => (
              <button key={method} onClick={() => setPaymentMethod(method)} style={{
                ...
                border: `1px solid ${paymentMethod === method ? "var(--color-primary)" : "var(--border)"}`,
                background: paymentMethod === method ? "var(--color-primary-light)" : "var(--surface)",
                color: paymentMethod === method ? "var(--color-primary)" : "var(--text)",
...
const panelStyle    = { background: "var(--surface)", borderRadius: 14, padding: 24, width: "100%", maxWidth: 420, border: "1px solid var(--border)", boxShadow: "var(--shadow)" };
const headingStyle  = { fontSize: 18, fontWeight: 600, color: "var(--text-h)", margin: 0 };
const headerRow     = { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 };
const closeBtnStyle = { background: "none", border: "none", fontSize: 22, color: "var(--text)", cursor: "pointer" };
...
const orderRow      = { display: "flex", justifyContent: "space-between", fontSize: 13, padding: "4px 0", borderBottom: "1px solid var(--border)" };
const orderTotal    = { display: "flex", justifyContent: "space-between", fontWeight: 500, fontSize: 14, paddingTop: 8, marginTop: 4, color: "var(--text-h)" };
const summaryBox    = { background: "var(--color-background-secondary)", borderRadius: 8, padding: "10px 14px", marginBottom: 14 };
const summaryRow    = { display: "flex", justifyContent: "space-between", fontWeight: 500 };
const summarySub    = { marginTop: 4, fontSize: 11, color: "var(--text)" };
const label         = { fontSize: 12, fontWeight: 500, color: "var(--text)", marginBottom: 8 };
...
const secondaryBtn  = { flex: 1, padding: "10px 0", borderRadius: 8, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text-h)", fontSize: 13, textAlign: "center", textDecoration: "none" };
